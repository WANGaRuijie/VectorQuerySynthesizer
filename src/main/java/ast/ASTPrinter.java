...
/**
 * A Visitor that creates a string representation of the AST with indentation
 * to visualize the tree structure. This is extremely useful for testing and debugging.
 */
public class ASTPrinter implements Visitor<String, Integer> {

    private String indent(int level) {
        // Return a string with `level * 2` spaces.
    ...
    public String visit(ProjectionNode node, Integer level) {
        ...
        sb.append(indent(level + 1)).append("Columns:\n");
        sb.append(visitChildren(node.getColumns(), level + 2));
    ...
    public String visit(ColumnReferenceNode node, Integer level) {
        return indent(level) + "ColumnRefNode(name=" + node.getColumnName() + ")\n";
    }
    ...
    // In a real project, you would implement visit methods for ALL nodes.
    // For this test, I'll add a few more common ones. The rest can be added as needed.

    // Defaulting other methods for brevity, a full implementation would cover all.
    ...
    @Override
    public String visit(DistanceExpressionNode node, Integer context) {
        return null;
    }
    ...
    @Override
    public String visit(BinaryOpExpressionNode node, Integer context) {
        return null;
    }

    @Override
    public String visit(CastExpressionNode node, Integer context) {
        return null;
    }

    @Override public String visit(NullValueNode n, Integer l) { return defaultVisit("NullValueNode", l); }
}
